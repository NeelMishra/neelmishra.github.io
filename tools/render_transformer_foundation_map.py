#!/usr/bin/env python3
"""Render a shared model map and compact locations for Foundations chapters.
...
LOCATIONS = {
    "embedding-layer": "Before the first block → token embeddings",
    "single-head-attention": "Inside a block → attention → one head",
    "multi-head-attention": "Inside a block → attention → combine the heads",
    "causal-attention": "Inside each head → mask scores before softmax",
    "feed-forward": "Inside a block → the MLP branch",
    "layer-norm": "Before each branch, and after the stack → normalization",
    "dropout": "On the update branches → optional training noise",
}


def render_location(slug):
    # The complete walkthrough belongs at the start and assembly of the model.
    # Other chapters need only a reminder of the component's location.
    return (f'<p class="tf-chapter-location" id="transformer-map">'
            f'{LOCATIONS[slug]}. '
            '<a href="index.html#transformer-map">View the model diagram →</a></p>')

# Slug, diagram label, and the component's input/output.
CHAPTERS = [
...
def main():
    ...
    for slug, label, description in entries:
        ...
        has_map = slug in ("index", "transformer-block")
        assets = CSS + '\n  '
        if has_map:
            assets += '<script defer src="../foundation-map.js?v=foundation-flow-20260911"></script>\n  '
        ...
        content = render_map(slug, label, description) if has_map else render_location(slug)
        html = replace_region(html, "MAP", content, r'(?=<span class="tr-anchor"|<h2)')
        # Earlier versions repeated the closing section in a recap callout.
        html = re.sub(r"<!-- FOUNDATION:RECAP -->.*?<!-- /FOUNDATION:RECAP -->\n?", "", html, flags=re.S)
