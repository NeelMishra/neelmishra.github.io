    ...
    all('[data-demo]').forEach(function (root) {
      ...
      function paint() {
        if (kind === 'bandit') {
          ...
          var gammaD = n(root, 'discount'), localD = [1+2*gammaD+3*gammaD*gammaD, 2+3*gammaD, 3], outerD = [1,gammaD,gammaD*gammaD], weightedD = localD.map(function (value,i) { return value*outerD[i]; });
          root.querySelector('[data-discount-value]').textContent = gammaD.toFixed(2);
          all('[data-local-return]',root).forEach(function (el,i) { el.textContent = f(localD[i]); });
          all('[data-outer-discount]',root).forEach(function (el,i) { el.textContent = f(outerD[i]); });
          all('[data-gradient-weight]',root).forEach(function (el,i) { el.textContent = f(weightedD[i]); });
          out.textContent = 'At γ = ' + gammaD.toFixed(2) + ', the gradient weights are (' + weightedD.map(f).join(', ') + '). Omitting the outer factor instead gives (' + localD.map(f).join(', ') + '). ' + (gammaD === 1 ? 'At γ = 1, both discount origins give the same weights.' : 'The outer factor keeps each remaining reward on the episode-start discount scale.');
